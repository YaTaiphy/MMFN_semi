...
def fit(model, dataloads, optimizer, criterion, device, batch_size, train=True):
    ...
    all_label = []
    all_pred = []
    for input_bert, input_swin, input_clip_text, input_clip_img, batchLabel in tqdm(dataloads, desc='进度', leave=True, ncols=100):
        step += 1
        ...
        detector_decode = model(
            input_bert.to(device), input_swin.to(device), input_clip_text.to(device), input_clip_img.to(device))
